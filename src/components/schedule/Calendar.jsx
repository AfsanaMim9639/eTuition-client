    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-4">
          <CalendarIcon className="w-8 h-8 text-[#FF10F0]" />
        ...
        <div className="flex items-center gap-2">
          <motion.button
            ...
            className="p-2 rounded-lg bg-[#121212] border border-[#FF10F0]/30 
                     hover:border-[#FF10F0] transition-all duration-300
                     hover:shadow-[0_0_20px_rgba(255,16,240,0.3)]"
          >
            <ChevronLeft className="w-5 h-5 text-[#FF10F0]" />
          </motion.button>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={goToToday}
            className="px-4 py-2 rounded-lg bg-[#121212] border border-[#00F0FF]/30 
                     hover:border-[#00F0FF] transition-all duration-300
                     hover:shadow-[0_0_20px_rgba(0,240,255,0.3)] text-white font-medium"
          >
            Today
          </motion.button>
          ...
          <motion.button
            ...
            className="p-2 rounded-lg bg-[#121212] border border-[#FF10F0]/30 
                     ...
                     hover:shadow-[0_0_20px_rgba(255,16,240,0.3)]"
          >
            <ChevronRight className="w-5 h-5 text-[#FF10F0]" />
          </motion.button>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="grid grid-cols-7 gap-2 mb-4">
          {dayNames.map((day, index) => (
            <div
              key={day}
              className="text-center py-3 text-sm font-semibold text-[#00F0FF]"
            >
              {day}
            </div>
          ))}
              ...
              return (
                ...
                >
                  <div className="flex flex-col items-center justify-center h-full">
                    <span className={`text-lg ${dayIsToday ? 'text-black' : 'text-white'}`}>
                      {date.getDate()}
                    </span>
                      ...
                      <div className="flex gap-1 mt-1">
                        {classes.slice(0, 3).map((cls, i) => (
                          <motion.div
                            ...
                            className={`w-1.5 h-1.5 rounded-full bg-gradient-to-r ${getStatusColor(cls.status)}`}
                          />
                        ))}
                        {classes.length > 3 && (
          ...
          >
            ...
            {getClassesForDate(selectedDate).length === 0 ? (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="text-center py-12 bg-[#121212] rounded-xl border border-white/10"
              >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="space-y-2">
                          ...
                          {cls.classType === 'online' && cls.meetingLink && (
                            <div className="flex items-center gap-2 text-sm text-white/70">
                              <Video className="w-4 h-4 text-[#FF10F0]" />
