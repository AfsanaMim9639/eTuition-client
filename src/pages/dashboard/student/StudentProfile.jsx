...
const StudentProfile = () => {
  ...
  useEffect(() => {
    ...
    setImagePreview(user.profileImage || 'https://i.ibb.co/qpB9ZNp/default-avatar.png');
  }, []);
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      // In real app, upload to cloud storage (Cloudinary, ImgBB, etc.)
...
<form onSubmit={handleSubmit} className="space-y-6">
    ...
    <div className="bg-gradient-to-br from-[#0f1512] to-[#0a0f0d] border-2 border-[#00ffcc]/30 rounded-xl p-6">
      ...
      <div className="flex items-center gap-6">
        <div className="relative">
          ...
          />
          <label className="absolute bottom-0 right-0 p-2 bg-gradient-to-r from-[#00ffcc] to-[#00ff88] rounded-full cursor-pointer hover:shadow-lg hover:shadow-[#00ffcc]/30 transition-all">
            <FaCamera className="text-[#0a0f0d]" />
            <input
              type="file"
              accept="image/*"
    ...
    <button
      type="submit"
      disabled={loading}
      className="w-full px-6 py-4 bg-gradient-to-r from-[#00ffcc] to-[#00ff88] text-[#0a0f0d] rounded-lg font-bold hover:shadow-lg hover:shadow-[#00ffcc]/30 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
    >
      {loading ? (
        <>
